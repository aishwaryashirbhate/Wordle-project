...
import os
import HW07_Aishwarya_Shirbhate_dictionary_final as dict
import HW07_Aishwarya_Shirbhate_wordle_final as w
import HW07_Aishwarya_Shirbhate_ui_final as ui
import HW07_Aishwarya_Shirbhate_occurence_stats_final as oss
import HW07_Aishwarya_Shirbhate_utility_final as uf
...
class wordle_test(unittest.TestCase):
    ...
    def test_case15(self):  # checks if all 5-letter words are returned to the function
        self.assertEqual(len(uf.five_letter_word()), 1379)

    def test_case16(self):  # checks if Add_to_freq_file works correctly
        tmp_file = 'tmp.csv'
        elements = ["a"] + [1, 2, 3, 4, 5]
        oss.Add_to_freq_file(tmp_file, elements)
    ...
    def test_case17(self):  # checks if Add_to_rank_file works correctly
        ...
        oss.Add_to_rank_file(tmp_file, elements)
    ...
    def test_case18(self):  # checks if Convert_list_to_tuple works correctly
        d = {"a": [1, 2, 3, 4, 5]}
        self.assertEqual(oss.Convert_list_to_tuple(d), {'a': (1, 2, 3, 4, 5)})
    ...
    def test_case20(self):  # checks if find_ranks works correctly
        ...
        n = 1379
        a = oss.find_ranks(words, occur_dict, n)
